...
/* GFMUL8: Carry-less 8x8 multiplication (opcode=0x3B, funct7=0x16) */
#define GFMUL8_HW(dest, a, b) \
    asm volatile ( \
        "addi t0, %[rs1], 0\n" \
        ".insn r 0x3b, 0x7, 0x16, %[rd], t0, %[rs2]\n" \
        : [rd] "=r" (dest) \
        : [rs1] "r" ((uint8_t)(a)), [rs2] "r" ((uint8_t)(b)) \
        : "t0", "cc" \
    )

/* GF_REDUCE: Reduce 16-bit polynomial mod 0x11D (opcode=0x3B, funct7=0x78) */
#define GF_REDUCE_HW(dest, x) \
    asm volatile ( \
        "addi t0, %[rs1], 0\n" \
...
uint16_t gf_mul(uint16_t a, uint16_t b) {
    ...
    //gf_carryless_mul(c, (uint8_t)a, (uint8_t)b);
        ...
        : [rd] "=r" (out) 
        : [r1] "r" ((uint8_t)a) , [r2] "r" ((uint8_t)b)
        : "cc", "t0"
    );
    //uint16_t tmp = (uint16_t)(c[0] ^ (c[1] << 8));
    uint16_t tmp = (uint16_t)(out & 0xFFFF);
