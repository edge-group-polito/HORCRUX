...
/* Execute OP_PRF_192: performs PRF at 192-level
              ...
              const uint32_t addr[8])
{
    ...
    for (i = 0; i < 8; i += 2) {
        cus_load(load32_prf(addr_bytes + 4*i),
                 load32_prf(addr_bytes + 4*(i+1)), 6 + i);
    }
    
    /* Load sk_seed to reg[14:19] (24 bytes = 6 words) */
    for (i = 0; i < 6; i += 2) {
        cus_load(load32_prf(ctx->sk_seed + 4*i),
                 load32_prf(ctx->sk_seed + 4*(i+1)), 14 + i);
    }
    ...
    /* Read result from reg[0:5] (24 bytes = 6 words) */
    for (i = 0; i < 6; i++) {
