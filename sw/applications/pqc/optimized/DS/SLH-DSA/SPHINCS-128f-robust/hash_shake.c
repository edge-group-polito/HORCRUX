              ...
              const uint32_t addr[8])
{
    ...
    cus_load(load32_prf(ctx->pub_seed + 0), load32_prf(ctx->pub_seed + 4), 0);
    cus_load(load32_prf(ctx->pub_seed + 8), load32_prf(ctx->pub_seed + 12), 2);
    
    /* Load addr to reg[4:11] (32 bytes = 8 words) */
    for (i = 0; i < 8; i += 2) {
        cus_load(load32_prf(addr_bytes + 4*i), load32_prf(addr_bytes + 4*(i+1)), 4 + i);
    }
    
    /* Load sk_seed to reg[12:15] (16 bytes = 4 words) */
    cus_load(load32_prf(ctx->sk_seed + 0), load32_prf(ctx->sk_seed + 4), 12);
    cus_load(load32_prf(ctx->sk_seed + 8), load32_prf(ctx->sk_seed + 12), 14);
