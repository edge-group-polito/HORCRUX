...
void hqc_pke_keygen(uint8_t *ek_pke, uint8_t *dk_pke, uint8_t *seed) {
    ...
    printf("hqc_pke_keygen hash_i cycles: %u\n", cycles_function);

    // Compute decryption key
    CSR_WRITE(CSR_REG_MCYCLE, 0);
    xof_init(&dk_xof_ctx, seed_dk, SEED_BYTES);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_keygen xof_init(dk) cycles: %u\n", cycles_function);

    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_sample_fixed_weight1(&dk_xof_ctx, y, PARAM_OMEGA);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_keygen vect_sample_fixed_weight1(y) cycles: %u\n", cycles_function);

    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_sample_fixed_weight1(&dk_xof_ctx, x, PARAM_OMEGA);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_keygen vect_sample_fixed_weight1(x) cycles: %u\n", cycles_function);

    // Compute encryption key
    CSR_WRITE(CSR_REG_MCYCLE, 0);
    xof_init(&ek_xof_ctx, seed_ek, SEED_BYTES);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_keygen xof_init(ek) cycles: %u\n", cycles_function);

    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_set_random(&ek_xof_ctx, h);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_keygen vect_set_random(h) cycles: %u\n", cycles_function);

    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_mul(s, y, h);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_keygen vect_mul(s=y*h) cycles: %u\n", cycles_function);

    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_add(s, x, s, VEC_N_SIZE_64);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_keygen vect_add(s=x+s) cycles: %u\n", cycles_function);

    // Parse encryption key to string
    CSR_WRITE(CSR_REG_MCYCLE, 0);
    memcpy(ek_pke, seed_ek, SEED_BYTES);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_keygen memcpy ek seed cycles: %u\n", cycles_function);

    CSR_WRITE(CSR_REG_MCYCLE, 0);
    memcpy(ek_pke + SEED_BYTES, s, VEC_N_SIZE_BYTES);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_keygen memcpy ek s cycles: %u\n", cycles_function);

    // Parse decryption key to string
    CSR_WRITE(CSR_REG_MCYCLE, 0);
    memcpy(dk_pke, seed_dk, SEED_BYTES);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_keygen memcpy dk cycles: %u\n", cycles_function);
...
void hqc_pke_encrypt(ciphertext_pke_t *c_pke, const uint8_t *ek_pke, const uint64_t *m, const uint8_t *theta) {
    ...
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_encrypt xof_init(theta) cycles: %u\n", cycles_function);

    // Retrieve h and s from public key
    CSR_WRITE(CSR_REG_MCYCLE, 0);
    hqc_ek_pke_from_string(h, s, ek_pke);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_encrypt hqc_ek_pke_from_string cycles: %u\n", cycles_function);

    // Generate re, e and r1
    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_sample_fixed_weight2(&theta_xof_ctx, r2, PARAM_OMEGA_R);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_encrypt vect_sample_fixed_weight2(r2) cycles: %u\n", cycles_function);

    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_sample_fixed_weight2(&theta_xof_ctx, e, PARAM_OMEGA_E);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_encrypt vect_sample_fixed_weight2(e) cycles: %u\n", cycles_function);

    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_sample_fixed_weight2(&theta_xof_ctx, r1, PARAM_OMEGA_R);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_encrypt vect_sample_fixed_weight2(r1) cycles: %u\n", cycles_function);

    // Compute u = r1 + r2.h
    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_mul(c_pke->u, r2, h);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_encrypt vect_mul(u=r2*h) cycles: %u\n", cycles_function);

    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_add(c_pke->u, r1, c_pke->u, VEC_N_SIZE_64);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_encrypt vect_add(u=r1+u) cycles: %u\n", cycles_function);

    // Compute v = C.encode(m)
    CSR_WRITE(CSR_REG_MCYCLE, 0);
    code_encode(c_pke->v, m);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_encrypt code_encode cycles: %u\n", cycles_function);

    // Compute v = C.encode(m) + Truncate(s.r2 + e)
    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_mul(tmp, r2, s);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_encrypt vect_mul(tmp=r2*s) cycles: %u\n", cycles_function);

    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_add(tmp, e, tmp, VEC_N_SIZE_64);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_encrypt vect_add(tmp=e+tmp) cycles: %u\n", cycles_function);

    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_truncate(tmp);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_encrypt vect_truncate(tmp) cycles: %u\n", cycles_function);

    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_add(c_pke->v, c_pke->v, tmp, VEC_N1N2_SIZE_64);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_encrypt vect_add(v=v+tmp) cycles: %u\n", cycles_function);
...
uint8_t hqc_pke_decrypt(uint64_t *m, const uint8_t *dk_pke, const ciphertext_pke_t *c_pke) {
    ...
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_decrypt hqc_dk_pke_from_string cycles: %u\n", cycles_function);

    // Compute u.y
    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_mul(tmp1, y, c_pke->u);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_decrypt vect_mul(tmp1=y*u) cycles: %u\n", cycles_function);
    // Truncate(u.y)
    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_truncate(tmp1);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_decrypt vect_truncate(tmp1) cycles: %u\n", cycles_function);
    // Compute v - Truncate(u.y)
    CSR_WRITE(CSR_REG_MCYCLE, 0);
    vect_add(tmp2, c_pke->v, tmp1, VEC_N1N2_SIZE_64);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_decrypt vect_add(tmp2=v+tmp1) cycles: %u\n", cycles_function);
    ...
    code_decode(m, tmp2);
    CSR_READ(CSR_REG_MCYCLE, &cycles_function);
    printf("hqc_pke_decrypt code_decode cycles: %u\n", cycles_function);
