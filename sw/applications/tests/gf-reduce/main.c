...
int main(void) {
    ...
    printf("GF Reduce Test (HQC GF(2^8) mod 0x11D)\n");

    // Golden reference is pre-computed in static golden[] array
    ...
    printf("[SW] Cycles for %d reductions: %u\n", N_TESTS, sw_cycles);
    ...
    printf("[HW] Cycles for %d reductions: %u\n", N_TESTS, hw_cycles);

    // ========== Verify SW vs Golden ==========
    printf("\n[VERIFY] Comparing SW results vs Golden...\n");
    for (int i = 0; i < N_TESTS; i++) {
        if (sw_results[i] != golden[i]) {
            printf("[SW] Test %2d FAIL: x=0x%04X golden=0x%02X sw=0x%02X\n",
                   i, test_inputs[i], golden[i], sw_results[i]);
            sw_ok = 0;
        }
    }
    if (sw_ok) {
    ...
    // ========== Verify HW vs Golden ==========
    printf("\n[VERIFY] Comparing HW results vs Golden...\n");
    for (int i = 0; i < N_TESTS; i++) {
        if (hw_results[i] != golden[i]) {
            printf("[HW] Test %2d FAIL: x=0x%04X golden=0x%02X hw=0x%02X\n",
                   i, test_inputs[i], golden[i], hw_results[i]);
            hw_ok = 0;
        }
    }
    if (hw_ok) {
