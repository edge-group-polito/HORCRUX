/**
 * @file main.c
 * @brief Kyber INTT Test - Software vs Hardware Comparison
 * 
 * Tests the Inverse Number Theoretic Transform (INTT) operation for Kyber
...
static int kyber_intt_sw_verify(const kyber_poly *r) {
    ...
    if (ok) printf("PASSED: Kyber INTT (Software)\n");
    return ok;
}
...

    {
        ...
        for (int j = 0; j < 128; j += 2) {
            ...
            asm volatile (
                ...
                : [a0] "r" ((int32_t)r->coeffs[j]),     [b0] "r" ((int32_t)r->coeffs[j+128]),
                  [a1] "r" ((int32_t)r->coeffs[j+1]),   [b1] "r" ((int32_t)r->coeffs[j+1+128]),
                  [z]  "r" (zeta)
                : "cc"
            );
            ...
            r->coeffs[j]       = (int16_t)s0;
            r->coeffs[j+128]   = (int16_t)s1;
            r->coeffs[j+1]     = (int16_t)s2;
            r->coeffs[j+1+128] = (int16_t)s3;
        }
    }
}
