/*
 * Gen Matrix - SOFTWARE IMPLEMENTATION
 * Uses pure software SHAKE128 (KeccakF1600_StatePermute)
 */
...
/* Rejection sampling */
static unsigned int rej_uniform_sw(int16_t *r,
                                   unsigned int len,
                                   const uint8_t *buf,
                                   unsigned int buflen)
{
...
static void sw_shake128_absorb_once(sw_xof_state *state, const uint8_t *in, size_t inlen)
{
  unsigned int i;
  ...
  /* Add SHAKE128 padding: 0x1F at position i, 0x80 at byte 167 MSB */
  state->s[i/8] ^= (uint64_t)0x1F << 8*(i%8);
  state->s[(SHAKE128_RATE-1)/8] ^= 1ULL << 63;
  state->pos = SHAKE128_RATE;
}

static void sw_shake128_absorb(sw_xof_state *state, const uint8_t *in, size_t inlen)
{
  unsigned int i;
  unsigned int pos = state->pos;

  while(pos+inlen >= SHAKE128_RATE) {
    for(i=pos;i<SHAKE128_RATE;i++)
      state->s[i/8] ^= (uint64_t)*in++ << 8*(i%8);
    inlen -= SHAKE128_RATE-pos;
    KeccakF1600_StatePermute(state->s);
    pos = 0;
  }
...
static void sw_shake128_finalize(sw_xof_state *state)
{
...
static void sw_shake128_squeezeblocks(uint8_t *out, size_t nblocks, sw_xof_state *state)
{
  unsigned int i;
...
/* XOF absorption */
static void xof_absorb_sw(sw_xof_state *state, const uint8_t seed[KYBER_SYMBYTES], uint8_t i, uint8_t j)
{
  sw_shake128_absorb_once(state, seed, KYBER_SYMBYTES);
  uint8_t ij[2] = {i, j};
  sw_shake128_absorb(state, ij, 2);
  sw_shake128_finalize(state);
}

#define GEN_MATRIX_NBLOCKS ((12*KYBER_N/8*(1 << 12)/KYBER_Q + XOF_BLOCKBYTES)/XOF_BLOCKBYTES)

/*
 * gen_matrix_sw - Software implementation
 */
static void gen_matrix_sw_impl(polyvec *a, const uint8_t seed[KYBER_SYMBYTES], int transposed)
{
  ...
  for(i=0; i<KYBER_K; i++) {
    for(j=0; j<KYBER_K; j++) {
      if(transposed)
        xof_absorb_sw(&state, seed, i, j);
      else
        xof_absorb_sw(&state, seed, j, i);

      sw_shake128_squeezeblocks(buf, GEN_MATRIX_NBLOCKS, &state);
      buflen = GEN_MATRIX_NBLOCKS*XOF_BLOCKBYTES;
      ctr = rej_uniform_sw(a[i].vec[j].coeffs, KYBER_N, buf, buflen);

      while(ctr < KYBER_N) {
        sw_shake128_squeezeblocks(buf, 1, &state);
        buflen = XOF_BLOCKBYTES;
        ctr += rej_uniform_sw(a[i].vec[j].coeffs + ctr, KYBER_N - ctr, buf, buflen);
      }
    }
  }
}
...
/* Public SW functions */
void gen_a_sw(polyvec *a, const uint8_t seed[KYBER_SYMBYTES])
{
  gen_matrix_sw_impl(a, seed, 0);
}

void gen_at_sw(polyvec *a, const uint8_t seed[KYBER_SYMBYTES])
{
  gen_matrix_sw_impl(a, seed, 1);
}
