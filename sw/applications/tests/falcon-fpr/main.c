...
{
    int all_passed = 1;
    uint32_t cycles_sw_fpr = 0;
    uint32_t cycles_hw_fpr = 0;
    ...
    CSR_READ(CSR_REG_MCYCLE, &cycles_sw_fpr);

    for (unsigned i = 0; i < FPR_TV_COUNT; i++) {
        if (got_vec[i] != FPR_TV[i].expected) {
            printf("[FPR ERROR] idx=%u s=%d e=%d m=", i, FPR_TV[i].s, FPR_TV[i].e);
            print_u64_hex(FPR_TV[i].m);
            printf(" got=");
            print_u64_hex(got_vec[i]);
            printf(" exp=");
            print_u64_hex(FPR_TV[i].expected);
            printf("\n");
            all_passed = 0;
        }
    }

    printf("SW cycles (FPR):        %u\n", cycles_sw_fpr);
    ...
    CSR_READ(CSR_REG_MCYCLE, &cycles_hw_fpr);

    for (unsigned i = 0; i < FPR_TV_COUNT; i++) {
        if (hw_full_vec[i] != FPR_TV[i].expected) {
            printf("[FPR HW ERROR] idx=%u s=%d e=%d m=", i, FPR_TV[i].s, FPR_TV[i].e);
            print_u64_hex(FPR_TV[i].m);
            printf(" got=");
            print_u64_hex(hw_full_vec[i]);
            printf(" exp=");
            print_u64_hex(FPR_TV[i].expected);
            printf("\n");
            all_passed = 0;
