...
int main(void)
{
    ...
    uint32_t cycles_sw_norm = 0;
    uint32_t cycles_hw_norm = 0;
    ...
    CSR_READ(CSR_REG_MCYCLE, &cycles_sw_norm);

    for (unsigned i = 0; i < NORM64_TV_COUNT; i++) {
        if (got_m_vec[i] != NORM64_TV[i].expected_m || got_e_vec[i] != NORM64_TV[i].expected_e) {
            printf("[NORM64 ERROR] idx=%u m=0x%016llX e=%d got_m=0x%016llX got_e=%d exp_m=0x%016llX exp_e=%d\n",
                   i,
                   (unsigned long long)NORM64_TV[i].m,
                   NORM64_TV[i].e,
                   (unsigned long long)got_m_vec[i],
                   got_e_vec[i],
                   (unsigned long long)NORM64_TV[i].expected_m,
                   NORM64_TV[i].expected_e);
            all_passed = 0;
        }
    }

    printf("SW cycles (FPR_NORM64): %u\n", cycles_sw_norm);
#else
    printf("Software tests skipped (SW_TEST_ENABLED=0).\n");
#endif

    CSR_WRITE(CSR_REG_MCYCLE, 0);
    all_passed &= run_norm64_hw_placeholder(hw_m_vec, hw_e_vec);
    CSR_READ(CSR_REG_MCYCLE, &cycles_hw_norm);

    for (unsigned i = 0; i < NORM64_TV_COUNT; i++) {
        if (hw_m_vec[i] != NORM64_TV[i].expected_m || hw_e_vec[i] != NORM64_TV[i].expected_e) {
            printf("[NORM64 HW ERROR] idx=%u m=", i);
            print_u64_hex(NORM64_TV[i].m);
            printf(" e=%d got_m=", NORM64_TV[i].e);
            print_u64_hex(hw_m_vec[i]);
            printf(" got_e=%d exp_m=", hw_e_vec[i]);
            print_u64_hex(NORM64_TV[i].expected_m);
            printf(" exp_e=%d\n", NORM64_TV[i].expected_e);
            all_passed = 0;
        }
    }
