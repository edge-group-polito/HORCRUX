/**
 * @file main.c
 * @brief Dilithium INTT Test - Software vs Hardware Comparison
 * 
 * Tests the Inverse Number Theoretic Transform (INTT) operation for Dilithium
...
static int dilithium_intt_sw_verify(const dsa_poly *a) {
    int ok = 1;
    for (int i = 0; i < DSA_N; i++) {
        ok &= compare_coeffs("D-INTT-SW", i, a->coeffs[i], dsa_tv_intt_out.coeffs[i]);
    }
    if (ok) printf("PASSED: Dilithium INTT (Software)\n");
    return ok;
}
...
static int dilithium_intt_hw_verify(const dsa_poly *a) {
    int ok = 1;
    for (int i = 0; i < 256; i++) {
        ok &= compare_coeffs("D-INTT-HW", i, a->coeffs[i], dsa_tv_intt_out.coeffs[i]);
    }
    if (ok) printf("PASSED: Dilithium INTT (Hardware)\n");
    return ok;
}
...
int main(void) {
    ...
    all_passed &= dilithium_intt_sw_verify(&a_sw);
    ...
    all_passed &= dilithium_intt_hw_verify(&a_hw);
