...
import os
import sys
sys.path.append('hw/vendor/x-heep/util')
...
def write_template(tpl_path, outfile, **kwargs):
    # Check if this is a crheepto template and add crheepto variables
    if tpl_path and 'crheepto' in str(tpl_path):
        # Try to find the crheepto config file
        config_path = pathlib.Path("config/crheepto-cfg.hjson")
        if config_path.exists():
            crheepto_vars = calculate_crheepto_variables(config_path)
            kwargs.update(crheepto_vars)
        else:
            # Default values if config not found
            kwargs.update({
                'xbar_nmasters': 0,
                'xbar_nslaves': 0,
                'periph_nslaves': 0,  # puffo, keccak, ascon from your config
                'ext_interrupts': 0   # from your config
            })
...
def calculate_crheepto_variables(cfg_path):
    """Calculate crheepto-specific variables from config file"""
    import hjson
    ...
    # Calculate crossbar masters
    if "ext_xbar_masters" not in cfg:
        xbar_nmasters = 0
    else:
        xbar_nmasters = len(cfg["ext_xbar_masters"])

    # Calculate crossbar slaves
    if "ext_xbar_slaves" not in cfg:
        xbar_nslaves = 0
    else:
        xbar_nslaves = len(cfg["ext_xbar_slaves"])

    # Calculate peripheral slaves
    if "ext_periph" not in cfg:
        periph_nslaves = 0
    else:
        periph_nslaves = len(cfg["ext_periph"])

    # Calculate external interrupts
